...
def vector_field_plt(odefunc, num_points, timesteps, inputs=None, targets=None,
    ...
    if len(save_fig):
        plt.savefig(save_fig, format='png', dpi=600, bbox_inches='tight')
        plt.clf()
        plt.close()
...
def multi_feature_plt(features, targets, save_fig=''):
    ...
    if num_dims == 2:
        fig, axarr = plt.subplots(1, len(features), figsize=(30, 10))
    ...
    if len(save_fig):
        fig.savefig(save_fig, format='png', dpi=600, bbox_inches='tight')
        plt.clf()
        plt.close()
    else:
        plt.show()
