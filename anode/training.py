...
class Trainer():
    ...
    def __init__(self, model, optimizer, device, classification=False,
        ...
        self.histories = {'loss_history': [], 'nfe_history': [],
                          ...
                          'epoch_accuracy_history': [], 'accuracy_history': []}
    ...
    def train(self, data_loader, num_epochs):
        ...
        for epoch in range(num_epochs):
            avg_loss, avg_acc = self._train_epoch(data_loader)
            if self.verbose:
                print("Epoch {}: {:.3f}".format(epoch + 1, avg_loss))
                print("Epoch {}: {:.3f}".format(epoch + 1, avg_acc))
    ...
    def _train_epoch(self, data_loader):
        ...
        for i, (x_batch, y_batch) in enumerate(data_loader):
            ...
            if self.classification:
                ...
                batch_correct = y_pred.eq(y_batch.data.view_as(y_pred)).sum()
                batch_accuracy = batch_correct.float() / len(y_pred)
            else:
                y_pred[y_pred > 0] = 1
                y_pred[y_pred <= 0] = -1
                batch_correct = y_pred.eq(y_batch.data.view_as(y_pred)).sum()
                batch_accuracy = batch_correct.float() / len(y_pred)
             # batch_accuracy = ()    # accuracy_score(y_pred, y_batch)
        ...
        self.histories['epoch_accuracy_history'].append(float(epoch_accuracy) / len(data_loader))
        if not self.is_resnet:
        ...
        return epoch_loss / len(data_loader), float(epoch_accuracy) / len(data_loader)
